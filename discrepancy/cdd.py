...
class CDD(object):
    ...
    def split_classwise(self, dist, nums):#############################把计算所得的距离通过类别区分开
    ...
    def gamma_estimation(self, dist):       ###############按类别进行gamma的计算
    ...
    def forward(self, source, target, nums_S, nums_T):
        ...
        for i in range(self.num_layers):
            ...
            cur_source = source[i]#############source的第二部分是probs
            cur_target = target[i]
            ...
            gamma_layers += [self.patch_gamma_estimation(nums_S, nums_T, dist)]#############gamma是什么东西？？？？？
