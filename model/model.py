...
class DANet(nn.Module):
    def __init__(self, num_classes, feature_extractor='resnet101', 
        ...
        self.feature_extractor = utils.find_class_by_name(
               ...
               frozen=frozen, num_domains=num_domains_bn)###############  backbones=[resnet]

        self.bn_domain = 0
        self.num_domains_bn = num_domains_bn

        feat_dim = self.feature_extractor.out_dim         ################## feat_dim=2048
        ...
        for k in range(self.num_hidden_layer):
            ...
            self.dropout1[str(k)] = nn.Dropout(p=cur_dropout_ratio)
        ...
        for k in range(self.num_hidden_layer):
            ...
            self.dropout2[str(k)] = nn.Dropout(p=cur_dropout_ratio)
            out_dim = fc_hidden_dims[k]
            self.FC2[str(k)] = FC_BN_ReLU_Domain(in_dim, out_dim, 
                  num_domains_bn)
            in_dim = out_dim
        ...
        self.dropout1['logits'] = nn.Dropout(p=cur_dropout_ratio)
        self.FC1['logits'] = nn.Linear(in_dim, num_classes)
        self.dropout2['logits'] = nn.Dropout(p=cur_dropout_ratio)
        self.FC2['logits'] = nn.Linear(in_dim, num_classes)
    ...
    def forward(self, x):
        ...
        x = feat
        for key in self.FC1:
            x = self.dropout1[key](x)
            x = self.FC1[key](x)
            to_select[key+'1'] = x

        to_select['probs1'] = F.softmax(x, dim=1)

        x = feat
        for key in self.FC2:
            x = self.dropout2[key](x)
            x = self.FC2[key](x)
            to_select[key+'2'] = x
            

        to_select['probs2'] = F.softmax(x, dim=1)
