...
def split_samples_classwise(samples, num_classes):################将样本地数据与标签按照类别存放
    data = samples['data'] 
    label = samples['label']
    gt = samples['gt']
    samples_list = []
...
def get_centers(net, dataloader, num_classes, key='feat'):        
    ...
    num_c = num_c.float().view(-1, 1)
    centers = centers.float()
    return centers/num_c
