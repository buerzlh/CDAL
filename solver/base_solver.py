...
class BaseSolver:
    ...
    def build_optimizer(self):
        ...
        param_groups_G = [{'params': self.net.module.feature_extractor.parameters(), 'lr_mult': 1.0}]
        param_groups_FC1 = [{'params': self.net.module.FC1.parameters(), 'lr_mult': opt.TRAIN.LR_MULT}]
        param_groups_FC2 = [{'params': self.net.module.FC2.parameters(), 'lr_mult': opt.TRAIN.LR_MULT}]

        assert opt.TRAIN.OPTIMIZER in ["Adam", "SGD"], \
            "Currently do not support your specified optimizer."

        if opt.TRAIN.OPTIMIZER == "Adam":
            self.optimizer['G'] = optim.Adam(param_groups_G, 
			lr=self.base_lr, betas=[opt.ADAM.BETA1, opt.ADAM.BETA2], 
			weight_decay=opt.TRAIN.WEIGHT_DECAY)
            self.optimizer['FC1'] = optim.Adam(param_groups_FC1, 
			lr=self.base_lr, betas=[opt.ADAM.BETA1, opt.ADAM.BETA2], 
			weight_decay=opt.TRAIN.WEIGHT_DECAY)
            self.optimizer['FC2'] = optim.Adam(param_groups_FC2, 
			lr=self.base_lr, betas=[opt.ADAM.BETA1, opt.ADAM.BETA2], 
			weight_decay=opt.TRAIN.WEIGHT_DECAY)
        elif opt.TRAIN.OPTIMIZER == "SGD":
            self.optimizer['G'] = optim.SGD(param_groups_G, 
			lr=self.base_lr, momentum=self.momentum, 
			weight_decay=opt.TRAIN.WEIGHT_DECAY)
            self.optimizer['FC1'] = optim.SGD(param_groups_FC1, 
			lr=self.base_lr, momentum=self.momentum, 
			weight_decay=opt.TRAIN.WEIGHT_DECAY)
            self.optimizer['FC2'] = optim.SGD(param_groups_FC2, 
			lr=self.base_lr, momentum=self.momentum, 
			weight_decay=opt.TRAIN.WEIGHT_DECAY)
    ...
    def test(self):
        ...
        for sample in iter(self.test_data['loader']):
            ...
            logits = (self.net(data)['logits1'] + self.net(data)['logits2'])/2
            preds += [logits]
            gts += [gt]
